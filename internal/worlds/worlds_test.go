...
func TestCompile_FacelessProjectIsTheDefaultWorld(t *testing.T) {
...
func TestCompile_ChainsAndFallback(t *testing.T) {
	...
	t.Run("rule 1: a faceless type is ABSENT, not excluded", func(t *testing.T) {
		for _, name := range []string{"published", "editorial"} {
			...
			assert.False(t, declared,
				"world %q must not carry a resolution for the faceless type", name)
		}
	})
	...
	t.Run("rule 2: the chain is ordered and type-filtered", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Equal(t, []entity.Face{ptr(t, "review"), ptr(t, "published")}, res.Chain)
	...
	t.Run("rule 3: a type the chain cannot satisfy compiles to an EMPTY chain", func(t *testing.T) {
		...
		// is the empty chain: a type declares faces, the world selects
...
`)
		...
		for _, tc := range []struct {
			...
			require.True(t, ok, "memo declares faces, so it must have a resolution")
	...
	t.Run("a chain coordinate the type lacks is dropped, not an error", func(t *testing.T) {
		...
		res, ok := scope.For("page")
		require.True(t, ok)
		assert.Equal(t, []entity.Face{ptr(t, "published")}, res.Chain)
	})
}
...
func TestCompile_RejectsBadFaceGrammar(t *testing.T) {
	tests := []struct {
		name string
		bad  string
	}{
...
`)
			...
			require.Error(t, err, "bad face name %q must not compile", tc.bad)
...
//
// The regression this catches is silent. A type whose face names all
// failed the grammar has an EMPTY entry in the declared-face map, and
// compileWorld reads an empty entry as rule 1 — "declares no faces,
