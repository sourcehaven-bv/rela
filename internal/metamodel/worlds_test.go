...
// worldsSchema is a schema fixture with two pointered types and one
// pointerless one, plus whatever `worlds:` block the test supplies.
...
// TestWorlds_ParseAndDeclare pins the happy-path shape of the `pointers:`
...
`)))
	...
	if got := len(m.Entities["page"].Pointers); got != 2 {
		t.Errorf("page pointers = %d, want 2", got)
	}
	if !m.Entities["page"].Pointers["draft"].Default {
		t.Error("page draft should be the declared default")
	}
	if m.Entities["page"].Pointers["published"].Default {
		t.Error("page published must not be the default")
	}
	if got := len(m.Entities["ticket"].Pointers); got != 0 {
		t.Errorf("ticket declares no pointers, got %d", got)
	}
...
// `pointers:` parses to empty declarations, so such a project behaves
...
`))
	...
	if m.Entities["ticket"].Pointers != nil {
		t.Errorf("Pointers = %v, want nil", m.Entities["ticket"].Pointers)
	}
}
...
`),
  ...
  ghost:
    ...
    otherwise: exclude
`),
			wantSubstr: []string{`world "ghost"`, `pointer "archived"`, "no entity type declares"},
		},
		{
...
`),
		...
		{
			name: "override names a pointerless type",
			schema: worldsSchema(`worlds:
  published:
    select: published
    overrides:
    ...
    otherwise: exclude
`),
			wantSubstr: []string{`world "published"`, `"ticket"`, "declares no pointers"},
		},
		{
			name: "override selects a pointer the type lacks",
			schema: worldsSchema(`worlds:
  published:
    select: published
    overrides:
    ...
    otherwise: exclude
`),
			wantSubstr: []string{`world "published"`, `pointer "review"`, `"page"`, "declares"},
		},
		{
			name: "edits names an undeclared pointer",
			schema: worldsSchema(`worlds:
  editorial:
    select: published
    otherwise: default
...
`),
		...
		{
			name: "two pointers marked default",
			schema: `version: "1.0"
namespace: https://example.org/test#
entities:
  page:
...
`,
			wantSubstr: []string{`entity "page"`, "default: true", "at most one"},
		},
	}
...
// naming a pointer only SOME types declare is legal — that is resolution
...
// same stance the pointer codec takes on multi-axis coordinates.
