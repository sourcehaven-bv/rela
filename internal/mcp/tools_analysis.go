...
// INVERSE id rather than the forward name prefixed with "incoming ". And a
// faced entity is checked per face (TKT-4Y6CMV) — the subject scan is
// AllStates, so a content-scoped edge on the draft no longer satisfies the
// published face's bound. Both bring this surface in line with what the CLI
// already reported.
func (s *Server) handleAnalyzeCardinality(
	ctx context.Context, _ *mcpgo.CallToolRequest,
) (*mcpgo.CallToolResult, error) {
	found, err := schema.CheckCardinality(ctx, s.deps().Store, s.deps().Meta, nil)
	if err != nil {
		return errorResult(err.Error()), nil
	}
...
// cardinalityMessage renders one violation as the sentence the MCP payload
// carries. It mirrors the wording `rela analyze cardinality` prints, so an
// agent reading the tool output and an operator reading the terminal see the
// same claim.
