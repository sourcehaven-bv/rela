...
func (h *writeHandler) handleV1CreateEntity(w http.ResponseWriter, r *http.Request, typeName, plural string) {
	...
	// Phase B: relation writes, addressed to the face just created — it owns
	// its content-scoped edges (BUG-64MU2Q).
	if req.Relations.Modern != nil {
		ws, err := h.applyRelationsModern(r.Context(), refOf(created), req.Relations.Modern)
