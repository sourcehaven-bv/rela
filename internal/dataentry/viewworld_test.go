...
func seedViewGraph(t *testing.T, app *App) {
	...
	if err := app.store.CreateEntity(ctx, &entity.Entity{
		ID: "TKT-1", Type: "ticket", Face: entity.Face("published"),
...
//
// It asserts on the entry's PROPERTIES rather than its face, because the
// face being right while the content is stale is not a failure mode this
...
func TestExecuteView_EntryResolvesToTheWorldsFace(t *testing.T) {
	...
	if pub.Entry.Face != entity.Face("published") {
		t.Errorf("entry face = %q, want published", pub.Entry.Face)
	}
}
...
func TestExecuteView_CollectionEntitiesAreTheWorldsFaces(t *testing.T) {
	...
	for _, e := range pub.Collections["features"] {
		...
		if e.Face != entity.Face("published") {
			t.Errorf("collected FEAT-PUB face = %q, want published", e.Face)
		}
		return
	}
...
func TestViewWorld_ProvenanceOnCollectionEntities(t *testing.T) {
	...
	if got.Face != "published" {
		t.Errorf("Face = %q, want the coordinate the face was stored at", got.Face)
	}
...
func TestViewWorld_ProvenanceDistinguishesFallbackFromChain(t *testing.T) {
	...
	w := viewWorld{name: "site-nl", scope: store.NewWorldScope(
		map[string]store.TypeResolution{
			"feature": {
				Chain:    []entity.Face{entity.Face("nl")},
...
func TestExecuteView_DefaultWorldUnchanged(t *testing.T) {
	...
	for _, e := range result.Collections["features"] {
		if e.Face != entity.Face("") {
			t.Errorf("%s: the default world serves default-state rows; got face %q",
				e.ID, e.Face)
		}
	}
}
