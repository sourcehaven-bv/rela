...
// TestPropertyIndexCacheSurvival tests that property index is correctly rebuilt
// when loading from cache
func TestPropertyIndexCacheSurvival(t *testing.T) {
	parameters := gopter.DefaultTestParameters()
	parameters.MinSuccessfulTests = 50

	properties := gopter.NewProperties(parameters)

	properties.Property("Property index rebuilt after cache load", prop.ForAll(
		func(entities []testEntity) bool {
			if len(entities) == 0 {
				return true
			}
			...
			for _, te := range entities {
				if expectedCounts[te.PropName] == nil {
					expectedCounts[te.PropName] = make(map[string]int)
				}
			...
			for propName, valueCounts := range expectedCounts {
				if g2.propertyIndex[propName] == nil {
					t.Logf("Property %s not in rebuilt index", propName)
					return false
				}

				for value, expectedCount := range valueCounts {
					actualCount := g2.propertyIndex[propName][value]
					if actualCount != expectedCount {
						t.Logf("Property %s value %s: expected count %d, got %d",
							propName, value, expectedCount, actualCount)
						return false
					}
				}
			}
