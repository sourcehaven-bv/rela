...
func TestWorldScope_AbsenceIsNotTheZeroValue(t *testing.T) {
	...
	assert.Equal(t, []entity.Face{entity.Face("published")}, res.Chain)
...
func TestNewWorldScope_CopiesInput(t *testing.T) {
	src := map[string]store.TypeResolution{
		"page": {Chain: []entity.Face{"published"}},
	}
	scope := store.NewWorldScope(src)

	src["page"] = store.TypeResolution{Chain: []entity.Face{"draft"}}
	src["policy"] = store.TypeResolution{Chain: []entity.Face{"draft"}}
...
func TestNewWorldScope_CopiesChains(t *testing.T) {
	t.Run("mutating the caller's chain does not re-scope", func(t *testing.T) {
		chain := []entity.Face{"published", "review"}
		scope := store.NewWorldScope(map[string]store.TypeResolution{
			"page": {Chain: chain},
		})

		chain[0] = entity.Face("draft")

		res, ok := scope.For("page")
		require.True(t, ok)
		assert.Equal(t, []entity.Face{"published", "review"}, res.Chain,
			"the scope must not share the caller's backing array")
	})

	t.Run("mutating a For result does not re-scope", func(t *testing.T) {
		scope := store.NewWorldScope(map[string]store.TypeResolution{
			"page": {Chain: []entity.Face{"published", "review"}},
		})
		...
		require.True(t, ok)
		first.Chain[0] = entity.Face("draft")

		second, ok := scope.For("page")
		require.True(t, ok)
		assert.Equal(t, []entity.Face{"published", "review"}, second.Chain,
...
func TestWorldScope_Types(t *testing.T) {
	scope := store.NewWorldScope(map[string]store.TypeResolution{
		"page":   {Chain: []entity.Face{"published"}},
		"policy": {Chain: []entity.Face{"review"}},
	})
