...
// faces but none the world selects — resolution rule 3 (design doc
...
type TypeResolution struct {
	...
	// ranked preference. Replacing the chain with `face IN (a, b)` would
	// return two rows for an entity holding both and break the
	// at-most-one-prime invariant everything leans on (design doc §4.2).
	//
	// That is a statement about REPLACING the chain, not about combining it
	// with a set filter. The backends already do both: worldSQL emits a
	// candidate PREDICATE (the chain ∪ fallback as an OR-set) and a rank
	// CASE, then DISTINCT ON picks the winner. ANDing a further face
	// allowlist beside the candidate — an ACL face grant, say — filters the
	// candidate set before ranking and still yields one row per id. Filtering
	// BEFORE the rank is also the semantics you want: an entity whose top
	// choice is denied falls through to the next candidate the reader may
	// see, rather than vanishing.
	Chain []entity.Face
...
// the only vocabulary a store speaks — entity type to ranked face
...
// every world (resolution rule 1: a type declaring no faces needs no
...
// parse or inspect one (see [entity.Face]).
//
