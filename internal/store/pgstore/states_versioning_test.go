...
// (TKT-DOFYR1): the sweep's `face = ''` / `from_face = ''` skips.
...
// entity_versions keys (entity_id, face, vseq) and every probe is
...
func TestSweep_CapturesStateTailedRelations(t *testing.T) {
	...
	rows, err := pool.Query(ctx,
		`SELECT rel_record_id FROM relations WHERE from_id = 'PAGE-2' ORDER BY from_face`)
	require.NoError(t, err)
	for rows.Next() {
		var id int64
	...
	// from_face column buys is the rename STITCH being able to tell them
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT count(*) FROM relation_versions WHERE rel_record_id = (
		   ...
		   WHERE from_id = 'PAGE-2' AND from_face = 'draft')`).Scan(&stateVersions))
	...
	require.NoError(t, pool.QueryRow(ctx,
		`SELECT from_face FROM relation_versions WHERE rel_record_id = (
		   SELECT rel_record_id FROM relations
		   WHERE from_id = 'PAGE-2' AND from_face = 'draft')`).Scan(&capturedTail))
...
//     differently.
//  2. The sweep's dedup probes are scoped `ev.face = e.face`, so a
