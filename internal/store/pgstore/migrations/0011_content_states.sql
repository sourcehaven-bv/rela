...
--
-- An entity may hold several content STATES addressed by (id, face);
-- the empty face is the default state, so every existing row already
-- is its own default state and this migration rewrites no data. The
-- face value is the canonical serialized coordinate produced by the
-- entity.Face codec; the store only ever equality-matches it, which
...
--
-- '' NOT NULL rather than NULL: the face joins both primary keys —
...
-- entities: the face joins the identity.
ALTER TABLE entities ADD COLUMN face TEXT COLLATE "C" NOT NULL DEFAULT '';
ALTER TABLE entities DROP CONSTRAINT entities_pkey;
ALTER TABLE entities ADD PRIMARY KEY (id, face);
...
-- so the uniqueness widens to (lower(id), face). Note what the index
-- does and doesn't enforce: cross-entity case collisions on the SAME
-- face slot are rejected here, but ('ABC','') alongside ('abc','draft')
...
-- stay entity-level, so there is deliberately no to_face).
ALTER TABLE relations ADD COLUMN from_face TEXT COLLATE "C" NOT NULL DEFAULT '';
ALTER TABLE relations DROP CONSTRAINT relations_pkey;
ALTER TABLE relations ADD PRIMARY KEY (from_id, from_face, rel_type, to_id);
...
-- old face-unaware definition. Each index is therefore REBUILT HERE,
...
--     strictly narrower (old-predicate AND face = ''), and every
--     existing row satisfies face = ''.
...
--     face = '' clause since this migration's release).
--
