...
func (s *Store) writeEntityTombstone(
	ctx context.Context, q DBTX, id string, p entity.Face, entityType string,
) error {
...
func (s *Store) writeRelationTombstone(
	ctx context.Context, q DBTX, from string, fp entity.Face, relType, to string,
) error {
...
func (s *Store) writeTombstonesForEvents(ctx context.Context, q DBTX, evs []store.Event) error {
	for _, ev := range evs {
		...
		case store.EventEntityDeleted:
			if err := s.writeEntityTombstone(ctx, q, ev.EntityID, ev.Face, ev.EntityType); err != nil {
				return err
			}
		case store.EventRelationDeleted:
			if err := s.writeRelationTombstone(ctx, q, ev.From, ev.Face, ev.RelationType, ev.To); err != nil {
				return err
			}
		}
	}
