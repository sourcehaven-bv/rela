...
func TestWorldSQL_ExcludeOmitsTheDefaultRowFromCandidates(t *testing.T) {
	...
	// rule-1 trailing arm (`NOT (type = $1) AND pointer = ''`) admitting
	...
	assert.NotContains(t, scopedArm(exclude), "pointer = ''",
		"exclude must not admit the scoped type's default row as a candidate")
	assert.Contains(t, scopedArm(dflt), "pointer = ''",
...
func TestWorldSQL_UnscopedTypesKeepTheirDefaultState(t *testing.T) {
	...
	assert.Contains(t, candidate, "pointer = ''",
...
func TestWorldSQL_IsDeterministic(t *testing.T) {
	...
	w := store.NewWorldScope(map[string]store.TypeResolution{
		...
		"note":  {Chain: []entity.Pointer{"live"}, Fallback: store.FallbackDefaultState},
		"draft": {Chain: []entity.Pointer{"wip"}, Fallback: store.FallbackExclude},
	})
...
func TestWorldSQL_AliasQualifiesEveryColumnReference(t *testing.T) {
	...
	// would corrupt any column whose name merely CONTAINS "pointer"
	// (from_pointer is one), and would silently miss a column added
	// later.
	...
	// bare "pointer"/"type" may survive, or PostgreSQL resolves it
	// against whatever else is in scope.
	for _, frag := range []string{" pointer", "(pointer", " type ", "(type "} {
	...
	assert.Contains(t, aliasCand, "e.pointer")
