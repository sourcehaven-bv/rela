...
// exists for that id. `pointer IN (a, b)` cannot express that — an entity
...
// name merely CONTAINS "pointer" — from_pointer is one — and would break
...
func worldSQL(w store.WorldScope, alias string, args *[]any) (rank, candidate string) {
		...
		*args = append(*args, typ)
		...
		for i, coord := range res.Chain {
			*args = append(*args, string(coord))
			c := len(*args)
			whens = append(whens, fmt.Sprintf("WHEN %s = $%d THEN %d", col("pointer"), c, i))
			coords = append(coords, fmt.Sprintf("%s = $%d", col("pointer"), c))
		}
		if res.Fallback == store.FallbackDefaultState {
			// Last resort: must rank BELOW every coordinate, hence
			// len(chain) rather than 0.
			whens = append(whens, fmt.Sprintf("WHEN %s = '' THEN %d", col("pointer"), len(res.Chain)))
			coords = append(coords, col("pointer")+" = ''")
		}
	...
	candidate = "(" + strings.Join(candArms, " OR ") +
		" OR (NOT (" + strings.Join(typeTests, " OR ") + ") AND " + col("pointer") + " = ''))"
