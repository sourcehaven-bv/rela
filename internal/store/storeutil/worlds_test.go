...
func ptr(t *testing.T, v string) entity.Face {
	t.Helper()
...
func cand(id, typ string, p entity.Face) storeutil.WorldCandidate {
	return storeutil.WorldCandidate{ID: id, Type: typ, Face: p}
}

// TestCompareStateKeys pins the ordering the whole world path depends on:
// state keys sort as the TUPLE (bare id, face), not as the joined
...
func TestWorldPrimes(t *testing.T) {
	...
	t.Run("rule 3: default falls back to the default state", func(t *testing.T) {
		got := storeutil.WorldPrimes(
			...
			[]storeutil.WorldCandidate{cand("PAGE-2", "page", def)})
		assert.Equal(t, map[string]entity.Face{"PAGE-2": def}, got)
	})
...
func TestPaginateWorldPrimes(t *testing.T) {
	...
	load := func(key string) (storeutil.WorldCandidate, bool) {
		...
		return storeutil.WorldCandidate{ID: id, Type: "page", Face: p}, true
	}
...
func TestWorldPrimes_MixedTypeFamilyIsOrderIndependent(t *testing.T) {
	...
	def := storeutil.WorldCandidate{ID: "A", Type: "other", Face: ""}
	pub := storeutil.WorldCandidate{ID: "A", Type: "page", Face: "published"}
...
func TestWorldPrimes_ZeroInChainIsAChainHit(t *testing.T) {
	scope := store.NewWorldScope(map[string]store.TypeResolution{
		"page": {
			Chain:    []entity.Face{"", ptr(t, "published")},
			Fallback: store.FallbackExclude,
		},
	})

	got := storeutil.WorldPrimes(scope, []storeutil.WorldCandidate{
		cand("PAGE-1", "page", entity.Face("")),
		cand("PAGE-1", "page", ptr(t, "published")),
	})

	if got["PAGE-1"] != entity.Face("") {
		t.Errorf("prime = %q, want the ZERO coordinate: it is first in the chain, so it "+
			"outranks published", got["PAGE-1"])
	}
}
