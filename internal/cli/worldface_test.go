...
// TestHasFace covers the adapter's content-state lookup — the content-state lookup B11 consumes.
//
// The ALIAS case is the load-bearing one: HasFace's doc claims it
...
// exercises that claim — the audit's own tests use a fake whose HasFace
...
func TestHasFace(t *testing.T) {
	...
	tests := []struct {
		...
		{"declared face", "page", "draft", true},
		{"second declared face", "page", "published", true},
		{"undeclared face on a faced type", "page", "review", false},
		{"ALIAS resolves to the canonical type", "pg", "draft", true},
		{"alias with an undeclared face", "pg", "review", false},
		{"a type with no faces block declares none", "ticket", "draft", false},
		{"the empty face is not a declaration", "page", "", false},
		{"undeclared type", "nosuchtype", "draft", false},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := (&metamodelReader{m: m}).HasFace(tc.entityType, tc.ptr); got != tc.want {
				t.Errorf("HasFace(%q, %q) = %v, want %v",
