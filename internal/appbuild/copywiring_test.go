...
func TestCopyDepsAreWired(t *testing.T) {
	...
	// EntityManager() returns a concrete *entitymanager.Manager since
	// TKT-IVSJV6 replaced the producer-side interface, so "the composition
	// root builds a real manager" is now a compile-time guarantee rather than
	// something to assert. What still needs proving is below: that the manager
	// it builds can actually run a guarded copy.
	...
	if res.Entity.Face != entity.Face("published") {
		t.Errorf("the copy must write the PUBLISHED face; got face %q",
			res.Entity.Face)
	}
	if !res.Created {
