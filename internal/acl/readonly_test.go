...
func TestReadOnlyACL_DeniesAllWrites(t *testing.T) {
	...
	tests := []struct {
		...
		{"create entity", acl.WriteRequest{Op: acl.OpCreate, Subject: acl.NewFacelessEntitySubject("ticket", "")}},
		{"update entity", acl.WriteRequest{Op: acl.OpUpdate, Subject: acl.NewFacelessEntitySubject("concept", "C-1")}},
		{"delete entity", acl.WriteRequest{Op: acl.OpDelete, Subject: acl.NewFacelessEntitySubject("person", "P-1")}},
		{"rename entity", acl.WriteRequest{Op: acl.OpRename, Subject: acl.NewFacelessEntitySubject("feature", "FEAT-1")}},
