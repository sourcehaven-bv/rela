...
|                       | `rela.url` helper, the document link rewriter, and             |
|                       | `rela-server routes`. Stdlib-only leaf. Parity with             |
|                       | `frontend/src/router/index.ts` enforced by `internal/frontendparity`. |
