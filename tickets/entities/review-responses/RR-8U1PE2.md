...
resolution: 'Plan updated (PR-A work list): the indexing observers SKIP events with a non-zero Face until Step 5 — the index remains exactly today''s default world, fail-safe. Pinned with a test in PR-A. Does not reopen the per-state-events decision (events still fire; the observer chooses).'
status: addressed
---
