...
severity: minor
resolution: Get is dropped — it had no consumer, and runDueTasks needs every configured task's state per tick anyway. All() is replaced by Load(ctx, tasks []string), scoped to the configured names so a shared table never returns another deployment's rows. Add Get back only when something (a CLI status command) actually needs one task.
status: addressed
---
