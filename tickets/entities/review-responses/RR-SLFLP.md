...
Dedup by (canonical relation, peer id) before emitting. A peer equal to the source id is the same physical edge under both keys; carry it once, under the canonical key, so the copy self-loops as the source did — or drop it with an `omitted` notice. AC18 asked whether the payload could be built so the conflict cannot arise; it can, but 'no error' turned out not to mean 'right answer'.
