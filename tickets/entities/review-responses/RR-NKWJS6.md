...
resolution: 'Mapping is explicit per operator x type, and unsupported cases ERROR rather than emit a silently-different predicate: fuzzy-with-wildcard (WildcardRe != nil) -> error; ordered/regex op on a list -> error; unknown property / non-integer-on-int / non-bool-on-bool -> error. List = maps to contains(); list != maps to len(list)>0 and not contains() (empty/missing list matches nothing, per filter). glob =/!= -> match()/not match(). Verified by TestFromFilter_UnsupportedRejected + the full parity matrix TestFromFilter_ParityWithFilterMatch (string/int/bool/date/list x eq/ne/empty/present/ordered/glob) across present/absent/empty records — identical verdicts to filter.Match. Added a len() host fn to distinguish empty from populated lists.'
status: addressed
---
