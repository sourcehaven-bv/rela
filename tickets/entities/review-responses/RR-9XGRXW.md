...
severity: critical
resolution: Migration is now import-once-then-delete the legacy key, and the ticket states plainly that this is not rolling-deploy-safe. An old binary rolled back after the import sees no state and treats everything as first-run — loud and obvious rather than the silent permanent divergence a retained key produced. The operational requirement (stop schedulers, deploy, start) goes in docs/postgres-backend.md; the scheduler is a single sequential component so a brief stop is far cheaper than duplicate side effects.
status: addressed
---
