...
finding: stateFamily adds a full entities-map scan (plus the existing relations scan) to every delete and rename; with idTaken a rename is ~3 full scans. A faceless 50k-entity project pays for a feature it doesn't use. entityOrder is sorted and states share the bare-id prefix, so binary search + prefix walk gives O(log n + k).
severity: minor
