...
resolution: 'Addressed in plan: history endpoints reuse gateReadOrNotFound/getVisible for live entities (inherits the RR-NGMI indistinguishable-404). For deleted entities gated by history:read, a NON-holder receives the SAME 404 as a nonexistent id (never a distinguishable 403). Snapshot-decode/store errors route through writeGateError so no table/column names leak. AC-2 includes the 404-oracle test analogous to the RR-NGMI cases in acl_get_test.go.'
status: addressed
---
