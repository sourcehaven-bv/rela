...
severity: minor
reason: 'Deferred to TKT-M5W1CY, not dismissed. The finding is real but describes a PRE-EXISTING limitation (RR-7408F5, already documented in ScheduledLuaWriteDeps'' own godoc) that this ticket did not introduce — it only made it observable by adding is_redacted(). Closing it means wiring an affordance resolver into appbuild and removing the deliberate NopRedactor substitution at appbuild.go:262. That is a change in ACL ENFORCEMENT SCOPE: scheduled jobs that today read hidden fields would stop reading them, which could break existing deployments'' jobs. Making that change as a side effect of a Lua ergonomics ticket would be the wrong forcing function and would land unreviewed in a PR whose stated subject is something else. Mitigated in the meantime by documentation: docs/lua-scripting.md now carries a runtime table stating explicitly that the scheduler always reports false, so nobody reads it as ''you are allowed to see this''.'
status: deferred
---
