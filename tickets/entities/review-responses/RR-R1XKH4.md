...
severity: minor
resolution: 'All three corrected. (1) Rewrote the reservation rationale to state what is true: a document named _export IS reachable today on all four shapes (verified by execution), and the reservation is forward-compatibility - a name that doubles as a route keyword is one dispatch change away from being silently shadowed, and that failure is a document that stops rendering with no error. (2) Corrected the dangling reference to TestExportSegmentPremise, and noted the alias between dataentry.exportSegment and dataentryconfig.ReservedExportSegment makes drift a compile error. (3) Moved the const above validateDocuments'' doc comment so each declaration owns its own documentation again.'
status: addressed
---
