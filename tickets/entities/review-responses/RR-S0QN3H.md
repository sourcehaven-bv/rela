...
reason: 'Deferred 2026-09-02 to TKT-ZEACWJ. Not a scope dodge: the fix is a GRAMMAR change to internal/predicate, which is shared by ACL affordances, state-machine transitions, automation conditions, wizard-form lint and validation rules. walkAttrGet requires a string-literal key on a RecordType and explicitly rejects computed keys, so numeric indexing needs a new walk case and IR node — and the package doc requires an accepted IR node to keep identical semantics across every evaluator and future SQL target. Landing that inside a webhook PR would put a five-surface language change through a review scoped to webhooks. Per-element iteration is folded into the same ticket because indexing alone still drops alerts[1..n] from a batched Alertmanager POST, so shipping indexing without iteration would look fixed while still losing alerts. Interim position is documented rather than silent: docs/webhooks.md carries a producer support matrix naming Icinga as working and Alertmanager/Grafana as not-yet, plus the warning that an unresolved reference is an empty string rather than an error. TKT-ZEACWJ is priority high.'
status: deferred
---
