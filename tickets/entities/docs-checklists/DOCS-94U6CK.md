...
- [x] Comments where logic isn't obvious (the load-bearing godoc set was a design deliverable: entity.Pointer's two rules, AllStates' not-world-resolution line, the observer skips, the FOR SHARE race rationale, migration 0011's rebuild reasoning, cursor restart semantics, RelationScope's IsIdentity warning)
- [x] Function/type docs if public API (Pointer/codec, GetEntityState, EntityQuery.AllStates, RelationQuery.FromPointer, RelationData.FromPointer, Event.Pointer, EntityHeader.Pointer, RelationScope + accessors, analysis.StateFinding/CheckStates, storeutil error constructors)
