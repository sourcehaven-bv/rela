...
- **Metamodel declaration + load-time validation** — PASS. `faces:`,
  `worlds:`, mandatory `otherwise:`, reserved `default` name, face
...
- **Default world costs nothing** — PASS. The default-world SQL is
  byte-identical to the pre-worlds shape (`WHERE face = ''`, no
