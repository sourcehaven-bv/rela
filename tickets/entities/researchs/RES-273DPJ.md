...
(face re-key becomes one migration op; FEAT-9CD2MX v1 ships only
undeclared-face *detection* pointing at it), so the shape needs deciding:
...
`rekey_id_prefix`, `rekey_face`. Pros: analyzable, dry-runnable, per-backend
...
key-shaped ops (type rename, id-prefix re-key, face re-key) are not
...
`history-purge`/`db migrate`. Key-shaped ops (type rename, id re-key, face
...
undeclared-face analyze finding, deliberately.
...
Sequencing note: the store-contract re-key ops are where FEAT-9CD2MX's face
