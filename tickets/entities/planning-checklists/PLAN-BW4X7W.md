...
- [x] ~~For larger features: run `/research`~~ (N/A: design of record already
      exists — `.ignored/face-design.md` §8, plus five prior review passes.
...
**Research Doc:** N/A — `.ignored/face-design.md` §8 is the design of
...
- **Consumer-side narrow view across an arch-lint boundary**:
  ...
  adapter in `internal/cli/acl.go:144`. Gains `HasWorld`/`HasFace`.
...
*Write grants* — `type@face` stays JOINED in `Create`/`Update`/`Delete`,
...
`parseStateGrant` reusing `entity.ParseFace` — NOT `entity.ParseStateRef`,
...
(adapter), `internal/metamodel` (exported face/world lookups — none exist
today; `anyTypeDeclaresFace` is unexported and is "any type").
...
| `update: ["T@p"]` | operator, `acl.yaml` | `entity.ParseFace` on the face half | hard load error |
| world name | operator, `schema.yaml` | **ALLOWLIST** `[a-z][a-z0-9]*(-[a-z0-9]+)*`, matching the face grammar | load refusal |
...
- [x] ~~`docs/data-entry.md`~~ (N/A: deferred by Ruling 7 — any UI surfacing is
  Jeroen's call, and none shipped in this epic.)
...
**Migration note owed:** an operator who adds `faces:` to an existing type
...
- [x] ~~All critical/significant findings addressed in plan~~ (N/A: superseded
  by the combined review on PR #1452. Two findings remain open and are carried
  there rather than here — RR-WEKTVV, needing a third arm for world-holding
  roles in the privileged-role refusal, and RR-SU4T68, whose PR-C scope needs
  re-deriving. Both are recorded in the body above.)
