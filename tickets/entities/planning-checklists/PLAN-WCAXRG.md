...
architect decisions (2026-08-19, incl. the Jeroen-confirmed Pointer-A
...
- **PR-A (contract core + fs/mem + storetest):** `entity.Pointer` named
...
`ValidateID`, pointer lexical `[a-z][a-z0-9-]*`, coordinate-ready signature,
never emits empty); `Entity.Pointer` + `Relation.FromPointer`; relation
`from_pointer:` frontmatter + `PAGE-1@draft.md` filename serialization; store
surface: `GetEntity` unchanged (= default state), new `GetEntityState(ctx, id
string, p entity.Pointer)`, `PutEntity` keys off `Entity.Pointer`, NO per-state
delete, `DeleteEntity`/rename cascade over ALL states + their relations;
`RelationQuery.FromPointer *entity.Pointer` (nil = unfiltered — the
...
`store.Event.Pointer`; fs/mem implementation incl. the
`relationMeta.FromPointer` index fix; storetest `RunStateTests` behind
transitional `Capabilities.States` (`// TODO(TKT-DOFYR1-PR-B): remove`); pgstore
compiles + fails closed on pointered writes.
- **PR-B (pgstore):** migration 0011 — compound PK `(id, pointer text
NOT NULL DEFAULT '')` on entities (Jeroen-confirmed), widened relations PK with
`from_pointer text NOT NULL DEFAULT ''`; `relationWhere` pointer match;
cascades; Event.Pointer + change-feed payload via codec; versioning checklist
(distinct rel_record_ids per FromPointer, pointer on sweep scan +
...
undeclared-pointer detection — analyze finding (count + example ids per
undeclared pointer) + appbuild load WARNING (never refusal), store stays
...
- Any worlds/resolver/metamodel-pointers work (Step 2), copy kernel
(Step 3+), ACL world grants (TKT-DN37J2), attachment link sets (§6.3).
- Re-key tooling for pointers — FEAT-T3EF5A's migration system;
DOFYR1 ships detection only (DEC-0VGTF3).

**Acceptance Criteria:**

1. Pointerless projects behave byte-identically: all 84 non-test
...
2. A state is addressable by `(id, pointer)` through the store surface;
...
implements.
4. Relation matching honors `FromPointer` in BOTH implementations
...
6. Events fire per state with `Event.Pointer`; pg change-feed payload
round-trips through the codec (PR-B).
7. Codec: parse/format round-trip, grammar rejection (bad base-id, bad
pointer, double `@`), never emits empty pointer; an unusual-but-canonical
pointer value round-trips the store unchanged.
8. Undeclared stored pointer → analyze finding + load warning, never a
refusal (PR-C).
...
restates), and fsstore's `relationMeta` index must gain `FromPointer` (one
...
on pointered writes in the window.
...
parse through the codec first — no route work in this ticket). Pointer grammar
...
relationMeta fix).
- Unusual-but-canonical pointer value round-trips (store opacity).
- `PAGE-1@draft.md` beside `PAGE-1.md`; file watcher sees state files.
- Event per state with correct Pointer.

**Negative Tests:**

- Codec rejects: bad base-id (defers to ValidateID), bad pointer chars,
uppercase pointer, leading digit/dash pointer, `a@b@c`, empty pointer
serialization.
- pg (window): pointered write rejected with clear error.
- Undeclared pointer: warning + finding, never refusal (PR-C).
...
- Godoc is the primary artifact (entity.Pointer, codec, store contract,
AllStates — load-bearing rules per decisions 4/6/8 + the amendment).
- [x] docs/metamodel.md — done in PR-C (relation `scope:` is the
first user-visible metamodel surface; pointers-per-type declaration is Step 2,
not this ticket)
...
pointer grammar tightened to `[a-z][a-z0-9]*(-[a-z0-9]+)*` exactly as proposed;
implemented in pointer.go.
- RR-R6G2VM (significant, addressed in plan): fs relation keys/filenames
must widen with the tail pointer (fsstore/relation.go:17 collision) — PR-A work
item + storetest case.
...
face; Step-1 indexing observers skip non-zero-Pointer events, pinned by test.
Per-world indexing stays Step 5.
- Minor (in plan, no entities): entity state files — filename
authoritative, no `pointer:` entity frontmatter key; verify loader behavior on
