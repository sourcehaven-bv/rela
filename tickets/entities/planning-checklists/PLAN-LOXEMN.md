...
1. type declares no faces → contributes its default state in every world;
2. type declares the face the world selects → that state is the prime; if
   the state does not exist, the entity **contributes nothing** (existence in
   a world *is* the publication bit);
3. type declares faces but none the world selects → the world's mandatory
   `otherwise: exclude | default`.

IN SCOPE (whole ticket):

- `entities.<type>.faces` and top-level `worlds:` metamodel declarations.
...
- `analysis.CheckStates` subtracting the declared face set from its
  `undeclared-face` finding.
...
- **Per-world search INDEXING and cross-world grouping** — Step 5, TKT-9KZGJO.
  The Step-1 deliberate skip (indexing observers ignore non-default-face
...
- **Face/axis re-keying** — rides the general data-migration system
...
1. **A metamodel with no `worlds:` block compiles to the zero `WorldScope`.**
   Test: load a fixture without `worlds:`/`faces:`, assert
...
3. **`select` / `overrides` may only name faces the type declares.** Test:
   a world selecting `published` for a type declaring only `draft` fails load,
   with an error naming world, type, and face.
4. **At most one `default: true` face per type**, and face names obey
   the `entity.ParseFace` grammar. Test: two defaults → load error; a
   face named `Draft` / `a--b` / `1x` → compile error naming file, type,
   offending name, and the grammar.
5. **`analyze states` subtracts declared faces, PER TYPE.** Test: a project
   declaring `draft` with rows stored under `draft` and `legacy` reports
   `undeclared-face` for `legacy` only.
   **[design review, RR-E1C216]** The declared set is a map `type → set`, not
   a set — `faces:` is per entity type. A flat subtraction would suppress
   `POLICY-1@draft` when only `page` declares `draft`, hiding exactly the
   stranded-data class that matters most (a faceless type contributes its
   default state in every world, so that row is reachable through NO world).
   `analysis/states.go:104-169` currently aggregates `byFace` globally;
   `stateRow.typ` (`:46-49`) already carries what is needed.
   **Decision for the implementer, not to be made at the keyboard:** filter
   per row at `states.go:119` with `if declared[st.typ][st.face]
   { continue }`, keeping the face-keyed headline finding and the existing
...
- [x] ~~For larger features: run `/research` to create a structured research doc~~
      (N/A: the design of record is `.ignored/face-design.md` v2 §4, already
...
**Research Doc:** N/A — design of record is `.ignored/face-design.md` §4;
...
**Not a row predicate.** `face IN ('review','published')` returns TWO rows
...
type TypeResolution struct { Chain []entity.Face; Fallback Fallback }
type Fallback int
...
- `otherwise: default` compiles to a **Fallback, not an appended chain
  ...
  rule-3-default with rule-2-on-a-declared-default-face.
- Rule 1 (faceless types) is expressed by **absence from `ByType`**, so a
  mixed graph costs zero per-type work.

*PR-A concretely (the unit being built now):*

1. `metamodel`: `EntityDef.Faces map[string]FaceDef` (`default: true`),
...
2. `metamodel` structural validation in the `validateRelationScope` mold,
   ...
   `select`/`overrides` naming declared faces, at most one `default: true`
...
3. `internal/worlds` (new package; arch-lint `mayDependOn: [entity, metamodel,
   ...
   plus `Default()`. **Face-grammar validation lives here** (Q6(b)), because
   `metamodel` may not import `entity` — with the requirement that the error
   names file, type, offending face name, and the grammar, i.e. is as good
...
4. `store.WorldScope` + `TypeResolution` + `Fallback` types only — nothing
   consumes them yet.
5. `analysis.CheckStates`: subtract the declared set from `undeclared-face`
   (`analysis/states.go:14-27` already promises this in its doc comment),
   per `(type, face)` and reading `*metamodel.Metamodel` — see AC5.
6. **[design review, RR-LLLBQY]** `internal/appbuild/appbuild.go:899-917`
   (`warnUndeclaredFaces`). Its two-COUNT probe warns that the store holds
   ...
   warning on "no type declares faces", preserving Step-1 behavior for
   faceless projects and going silent for adopters. Update the doc comment.
7. **[design review, RR-CGRV0X]** `validateWorlds` also cross-checks
   ...
   types that actually declare faces. Load-time only, cheap, does NOT
...
- `// family-scoped write path — deliberately face-unaware, do NOT world`
...
Enumerate via `grep -rn "face = ''\|from_face = ''"
...
picked before the gate is consulted), `Resolved{Entity, Face, Fallback}`
...
`FromFace` has no "this-state-or-identity" value. Worse, identity edges
and default-state content edges are **stored identically** — both
`from_face = ''`, no discriminator (`fsstore/relation.go:132-135`,
...
content-scoped types with the **prime's face**; results merged. One
...
1. **The fallback trap, explicitly handled and tested.** When
   `otherwise: default` fires, the prime's face IS the zero face —
   which as a `FromFace` value means "default-tail ONLY", a **different
...
| --- | --- |
| World as a `(type, face)` set predicate | Breaks at-most-one-prime; cannot express ordered fallback or "contributes nothing". §2.1 of the survey. |
...
- `internal/metamodel/types.go` — `Metamodel.Worlds`, `EntityDef.Faces`,
  `WorldDef`, `FaceDef`, `Otherwise` type + `IsValid()`.
- `internal/metamodel/loader.go` — `validTopLevelKeys` (+`worlds`),
  `validate()` wiring, `validateWorlds` / `validateFaces`.
...
- `docs/metamodel.md` — `faces:` / `worlds:` reference (see Doc Planning).
...
| --- | --- | --- | --- |
| `worlds:` block | `schema.yaml` — operator-authored, in-repo | Structural checks in `metamodel` (mandatory `otherwise:`, chain references a declared face, unique names, `default` reserved) | **Hard load error**, collected so all problems surface at once |
| face names | `schema.yaml` | `entity.ParseFace` **allowlist grammar** `^[a-z][a-z0-9]*(-[a-z0-9]+)*$`, applied in `internal/worlds` (Q6b) | Compile error at startup naming file/type/name/grammar |
...
| stored `face` column/filename | storage | Already validated by the DOFYR1 codec; the store equality-matches and never inspects | unchanged |

Note the CLAUDE.md rule: **the configuration is not a secret.** World names,
face names and `otherwise:` values are operator-authored config in a
...
4. **No SQL is built from face or world values.** Chains travel as bound
...
| No `faces:` on any type | Same as above; rule 1 for every type |
| Mixed graph (faceless `ticket` beside faced `page`) | `ticket` absent from `ByType`, contributes in every world; no special handling |
...
| `overrides` naming a type with no faces | Load error (rule 1 types have no chain to override) |
...
| Cursor/pagination across a family boundary | Families are contiguous under `(id, face)` ordering, so the buffer window is ONE family, not the result set — pinned as a conformance case |
| `AllStates` + `World` set together | `store.ErrInvalidQuery` from all three backends (AC9) |
| Very long chain / many types | Bounded by declared faces; no unbounded growth (§7's "bounded-and-declared" argument) |
| Unicode / uppercase / `--` in a face name | Rejected by the grammar (`ParseFace` already rejects `--` because the face serializes into the relation-key FROM slot) |
...
- World `select`ing an undeclared face → load error naming world, type,
  face.
- `overrides` naming an unknown entity type → load error.
- Two faces with `default: true` on one type → load error.
- Bad face grammar → compile error naming file, type, name, grammar, at
  startup.
...
| **[design review] Boot warning goes false** (RR-LLLBQY) — `warnUndeclaredFaces` warns about declared drafts from PR-A onward | Significant | Gate on "no type declares faces"; update the doc comment |
...
| **`face = ''` spells two different concepts** (default world vs identity anchor) | Medium | PR-C required deliverable: disambiguating comment at every literal. Q5 settled: CTE seeds are the identity anchor and stay un-worlded |
| **Performance of the world query on a large type** — the join is `(type, face)`, the DISTINCT ON is `id`; existing indexes are `entities_type_idx` and PK `(id, face)` | Medium | Measure, don't speculate: `graphquery_explain_test.go` + `graphquery_bench_test.go` already exist as the harness. A new index is a decision made on evidence, not a speculative migration |
| **fs/mem buffering** — per-family candidate buffering during a list | Low | Only ever runs for a NON-default world; `IsDefaultWorld()` short-circuits to today's path. Bounded to one family by `(id, face)` contiguity |
| **Regression for faceless projects** | Low but unacceptable if it happened | Zero-value fast path is structural, not incidental; AC1 + AC6 pin it; the faceless project never allocates a map |
...
- [x] `docs/metamodel.md` — **new `faces:` and `worlds:` reference**: the
...
- **RR-7FOWDB — search is a fifth world-scope site and a second `AllowAll`
  ...
  hardcoding `e.face = ''`; `search/visible.go:269` routes through
