...
2. The godoc says why per-scope is not merely unimplemented but unavailable, and
   names the concrete precondition for changing that. Scenario: a future reader
   who wants to narrow the scope learns from the doc that the tombstone row is
   the blocker, and that `writeEntityTombstone` is what would have to change
   first — so they do not start the work before facing the GDPR question.
...
No library question here; this is prose about this codebase's own trade-off.
The research was verification of the ticket's factual claims against the code,
...
- `internal/store/store.go:585-625` — the `TypeWatermark` godoc. Confirmed it
  ...
  client forever) and closes with "Do not 'optimize' this into a per-collection
  or per-principal scope without solving the tombstone problem first."
  Confirmed it never mentions confidentiality, principals-as-adversaries, or
  what a shared tag discloses. The gap the ticket describes is real.
- `internal/store/pgstore/tombstone.go:13-17` — `writeEntityTombstone` executes
  `INSERT INTO deletions (kind, id_a, typ) VALUES ('e', $1, $2)`. Confirmed:
  id and type only, no relations, no properties. The "narrower scope cannot be
...
- `internal/dataentry/caldav_backend.go:655-676` — `watermarkCTag`. Confirmed it
  ...
  VALUES but leaves `seq` the only varying input, so the tags still MOVE
  together. That distinction is the whole finding and is worth stating.
...
derivable from the surrounding code. A reader of `tombstone.go` sees a two-column
insert and could easily think "adding a third column is cheap".
...
- The disclosure: `EntityTypeWatermark(cfg.EntityType)` is the only varying
  ...
  lock-step, across ACL boundaries. Confirmed against
  `caldav_backend.go:655-676`.
...
- A type that has never had a row returns 0 (documented in the method comment),
  a stable value that discloses "nothing of this type exists yet". Already
  covered by the existing text; not re-litigated.
...
**Effort:** xs. Confirmed against the ticket's own estimate; no reason to revise.
...
- [ ] `docs/data-entry.md` — describes the `caldav:` config block; the config
      surface is unchanged, and the caveat belongs with the deployment guide
      rather than the key reference.
