...
description: |
  ...
  - Go HTTP API in `internal/dataentry`: REST under `/api/v1/` plus a small
    set of legacy `/api/*` endpoints (toggle-checkbox, help, command/cancel,
    open-file, git status/sync) still consumed directly by the SPA, plus two
    SSE streams (`/api/events`, `/api/v1/_events`) for live reload
