...
prevention: Route the dataentry list filter through propmatch/filter.Match rather than a local fmt.Sprintf comparison, and add HTTP-level table tests covering list-typed properties for eq/ne/in/contains. Consider a lint or arch guard flagging fmt.Sprintf("%v") used for property-value comparison outside propmatch, and audit remaining read-path call sites for the scalar-only assumption already fixed on the write path.
status: in-progress
---
