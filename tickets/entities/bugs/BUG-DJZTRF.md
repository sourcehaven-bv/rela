...
prevention: 'Pin the anti-flash contract with component tests that observe the INTERMEDIATE render rather than the settled state - the empty state must never appear mid-fetch. Assert on the rendered body''s CHILD node, not the container: v-html replaces a container''s contents, so container identity survives either way and a test asserting on it pins nothing (this exact mistake was made first time round and caught in review, RR-7DHGWF). Spy on the diagram watcher as a second, independent signal that no repaint occurred. Fence every async render with a monotonic generation counter so a superseded response cannot paint, raise a toast, or clear loading - removing the blank that used to make an out-of-order render visible is what turned a latent race into a reachable one. See AM-document-rerender-preserves-content.'
status: done
---
