...
The version schema is fully `(entity_id, face)`-keyed — `insertVersion`
writes the face as column 2 and folds it into the content hash
(`pgstore/version.go:44-88`), and lineage reads filter
`AND face = CAST($2 AS text)`. Per-state versioning landed in TKT-C1XUA8
...
**returns early for any edge whose `FromFace` is non-default**, and the
