...
prevention: 'P2: normalise nil-ness where the boxing happens — newAliasFanout drops any subscriber that is nil, including a typed nil. This settles alias subscribers only; the same services also reach Services as concrete fields, where each consumer still nil-checks separately, so it is not a system-wide chokepoint. The structural fix is to make "disabled" unrepresentable as a nil pointer (an explicit Optional/enabled flag from buildComments) — larger than this bug warrants, recorded here rather than implied. P4 (automated measure): regression tests that feed the real buildComments output (not a nil literal) into newAliasFanout, both disabled and enabled.'
status: done
---
