...
prevention: 'The upstream commits 529671b (#342) and 2b1fc21 (#347) removed App.mu, reloadLockMiddleware, and the lock-upgrade dance from dataentry — the exact architectural smell this ticket described. Parallel work (post-rebase verification of BUG-D2QS via the stress harness) showed the mutation-mode hang symptom disappears after those commits land. The original user-reported ''long long time'' Firefox hang was never directly reproduced, but if it shared a root cause with BUG-D2QS then it is now fixed by the upstream refactor rather than by anything in this ticket. The wont-fix status stands: nothing in this ticket''s original acceptance criteria is now necessary. The stress harness (frontend/stress/) remains as the standing regression guard — run it with --mutate before any PR that touches dataentry concurrency.'
status: wont-fix
---
