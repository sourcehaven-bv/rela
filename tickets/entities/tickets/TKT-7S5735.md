...
> **Planning is complete** (PLAN-6X0Y7W, status `done`) and passed
> `/design-review` — ten findings, three of them critical, all addressed in the
> plan. Implementation has NOT started.
>
> The ticket sits in `backlog` rather than `planning` deliberately: the
> `ci-no-planning-tickets` gate assumes a ticket's PR carries its
> implementation, so a planning-only PR cannot merge in any non-terminal state
> except `backlog`. Move it to `in-progress` when implementation begins — that
> transition auto-creates the implementation checklist.
>
> **Read PLAN-6X0Y7W before starting.** The design in "Proposed architecture"
> below is the user-agreed shape, but the first planning draft of *how* to build
> it was wrong in a way that would have reproduced the BUG-FB0LN8 failure mode:
> visibility is computed from `formData` (`DynamicForm.vue:212`), so deciding
> "which fields would hide" requires hypothetical evaluation, not a mutation
> followed by a dialog. That correction is the centrepiece of the plan.
