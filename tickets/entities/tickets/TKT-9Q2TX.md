...
> **Closed by backlog sweep (2026-07-20):** the race this fixes cannot occur anymore — `a.Graph().GetNode(id)` live-node mutation is gone; handlers read via store (every backend returns a fresh Clone, e.g. memstore.go GetEntity) and mutate via entityManager.UpdateEntity.
