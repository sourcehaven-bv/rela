...
12. Mixed-group entity: one file can hold properties encrypted for multiple groups (e.g., `description: engineering` + `secret: exec`). **One data key per group** is generated: properties encrypted for group A are sealed with data key A, properties encrypted for group B with data key B. This gives within-file group isolation — a recipient of group A cannot decrypt group B's properties even though both envelopes live in the same file. `_encryption.data_keys` contains one section per referenced group, each wrapping that group's own data key for its recipients. (Earlier plan drafts proposed a single per-file data key; the end-to-end demo revealed that it broke group isolation. Corrected in-branch before merge.)
