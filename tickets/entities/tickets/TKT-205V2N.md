...
id: TKT-205V2N
type: ticket
title: related() in views, next-action, CLI filter, validation, automation and state machine
kind: enhancement
priority: medium
effort: xl
status: ready
...
4. Refuse `related()` at load for form conditions (evaluated in the browser)
and ACL affordance `when:`.

Out of scope: affordances keep their own bindings; ACL support for `related()`
comes later.
