...
- **Rename consistency.** `renameAttachmentDir` (`internal/store/fsstore/attachment.go:206`) moves the directory and re-keys the in-memory index, but does **not** re-stamp the entity's `property` value, which still holds the old `attachments/<oldID>/...` string. That stale face is both a latent correctness bug (frontmatter points at a moved path) and an ACL-soundness hazard (file associated with a historical ID). The access path must resolve via the **canonical current entity ID**; either re-stamp the property on rename or have the read path ignore the literal stored string and reconstruct from `(currentEntityID, property)`. Add a rename test.
