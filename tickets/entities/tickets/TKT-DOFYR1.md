...
- `entity.Entity` gains `Face` (`""` = default state); relations gain a `from_face` tail; heads stay entity-level (§2.3).
...
- Relation matching gains the face in BOTH implementations: `storeutil.MatchRelation` (fs/mem) and pgstore `relationWhere` SQL (`relation.go:298-335`) — v1's "single choke point" claim was false.
