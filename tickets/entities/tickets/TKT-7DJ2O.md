...
> **Sweep note (2026-07-20): the snapshot-prologue is now the documented norm (CLAUDE.md capture-state-once rule; internal/dataentry/app.go says handlers call a.State() once at entry; 28 of 53 handler funcs comply). The body's named offenders reference the dead `a.Graph()` API. Remaining real scope: bring the stragglers onto the prologue and add the missing coherence test (TestHandlerCoherenceUnderReload does not exist).**
