...
non-default state row `(id, face<>'')` references the family's default row `(id,
'')` (e.g. via a generated/constant column), with `ON DELETE CASCADE` and `ON
UPDATE CASCADE`. That makes headless states UNREPRESENTABLE in pg and gives the
family delete/rename cascade for free, replacing the probe.
...
Closed: Obsolete: BUG-HC6I2T (#1557) removed the head face this invariant was
about.
