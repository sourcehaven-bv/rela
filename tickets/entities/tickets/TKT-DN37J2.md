...
- Write grants per `type@face` (codec-parsed at policy load), no inheritance, fail closed; no role holds update on published.
...
- Cross-file validation (grants naming declared faces/worlds, scope names) in `internal/aclaudit`, not `internal/acl` (arch-lint forbids acl → metamodel).
