...
/**
 * What the node already carries, used when the resolver has no answer.
 *
...
function resolveAttrs(
  ...
  try {
    ...
    // A throwing resolver is treated as "no answer", matching
    // rewriteEntityRefToken. The reference keeps whatever it had.
    return keepExisting(node)
  }
  // No entry means the server did not resolve this ID for this principal —
  // either it is unreadable, it does not exist, or it postdates the map. None
  // of those is a reason to discard a title the node already holds.
  if (!hit || !hit.type) return keepExisting(node)
  return {
