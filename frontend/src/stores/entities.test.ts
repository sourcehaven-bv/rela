...
describe('Entities Store', () => {
  ...
  describe('world-scoped caching', () => {
    ...
    // holds two copies of one face that can disagree.
        // On a deployment with `default_world`, an ABSENT param means the
    // CONFIGURED world and `default` means the bare faces — two different
    // faces of the same entity. So `default` is sent verbatim and cached
    // apart from the absent spelling; collapsing them served one face as the
    // other. Callers pass `useWorld().worldParam`, which only spells `default`
    // when a configured default makes the distinction real.
    it('sends an explicit "default" world verbatim and caches it apart', async () => {
      vi.mocked(entitiesApi.getEntity).mockResolvedValue(publishedFace)
      ...
      expect(entitiesApi.getEntity).toHaveBeenLastCalledWith('policy', 'POL-1', { include: '*' })
      expect(entitiesApi.getEntity).toHaveBeenCalledTimes(2)
    })
