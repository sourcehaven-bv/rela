...
function setup(opts: {
  ...
  return { policy: useChangePolicy(deps), form, retained, applied, hiddenBatches }
}
...
describe('useChangePolicy', () => {
  it('applies an accepted proposal', () => {
    const { policy, form, applied } = setup({
      form: { route: 'aanbesteding', deadline: '2026-09-15' },
      visibleFor: routeRule,
    })
    expect(policy.propose('route', 'onderhands', 'aanbesteding')).toEqual({ status: 'applied' })
    expect(form.route).toBe('onderhands')
    expect(applied).toHaveLength(1)
  })

  it('reports the fields a proposal would hide', () => {
    const { policy, hiddenBatches } = setup({
      form: { route: 'aanbesteding', deadline: '2026-09-15' },
      visibleFor: routeRule,
    })
    policy.propose('route', 'onderhands', 'aanbesteding')
    expect(hiddenBatches).toEqual([['deadline']])
  })

  it('reports nothing hidden when visibility is unchanged', () => {
    const { policy, hiddenBatches } = setup({
      form: { route: 'aanbesteding', deadline: '2026-09-15' },
      visibleFor: routeRule,
    })
    policy.propose('deadline', '2026-10-01', '2026-09-15')
  ...
  it('retains a hidden field s value before applying the change', () => {
  ...
  it('retains nothing when the change hides nothing', () => {
    const { policy, retained } = setup({
      form: { route: 'aanbesteding', deadline: '2026-09-15' },
      visibleFor: routeRule,
    })
  ...
  it('retains the PRE-change value when a field hides itself', () => {
    ...
    policy.propose('mode', 'summary', 'detail')
  ...
  // defence.
  it('retains before applying, never after', () => {
    const order: string[] = []
  ...
  it('retains every field in a multi-field hide', () => {
  ...
  it('does nothing when disabled (create path)', () => {
  ...
  it('ignores an unmanaged property that stops being visible', () => {
    const { policy, retained, hiddenBatches } = setup({
  ...
  it('hidesFrom does not mutate or apply anything', () => {
    const apply = vi.fn()
