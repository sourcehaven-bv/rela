 ...
 * same-entity one, whose target is the source by construction. Every offer in
 * `entity._copies` is same-entity, so the SPA never passes it; the parameter
 * exists for a caller that knows a definition and a target by other means.
 *
