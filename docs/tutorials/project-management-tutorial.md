...
- Automations to enforce workflows and reduce manual tracking
...
## Part 11: Automating Workflows

Rela's automation engine lets you define trigger-action rules that fire when
entities change. This reduces manual work and enforces consistency.

### Adding Automations to Your Metamodel

Add an `automations` section to your `metamodel.yaml`:
...
automations:
  ...
  track-work-start:
    ...
    description: Auto-set started_at when work begins
    on:
      entity: [task, feature]
      property: status
      becomes: in_progress
  ...
  track-work-completion:
    name: Track Work Completion
    on:
      entity: [task, feature, bug]
      property: status
      becomes: done
    do:
      - set: completed_at
  ...
  # Ensure blocker bugs have assignees
  blocker-requires-assignee:
    name: Blocker Bug Escalation
    on:
      entity: bug
      property: severity
      becomes: blocker
    validate:
      - check: assignee != ""
        severity: error
        message: "Blocker bugs must have an assignee"
  ...
  decision-completeness:
    ...
    validate:
      - check: rationale != ""
        severity: error
        message: "Decided decisions must document rationale"
      - check: consequences != ""
        severity: warning
        message: "Consider documenting consequences"
      - check: decided_date != ""
        severity: error
        message: "Decided decisions must have a date"

  # Risk materialization tracking
  risk-occurred-check:
    name: Risk Occurred Tracking
    on:
      entity: risk
      property: status
      becomes: occurred
    validate:
      - check: contingency_plan != ""
        severity: warning
        message: "Activate contingency plan for occurred risks"

  # New task assignment
  task-created-defaults:
...
Automations support these template variables:

| Variable | Description | Example |
|----------|-------------|---------|
| `{{today}}` | Current date (YYYY-MM-DD) | `2024-02-15` |
| `{{now}}` | Current timestamp (ISO 8601) | `2024-02-15T10:30:00Z` |
| `{{user.name}}` | Current user (from git config) | `Alice Smith` |
| `{{user.email}}` | User email (from git config) | `alice@example.com` |
| `{{entity.id}}` | Current entity ID | `TASK-001` |
| `{{entity.type}}` | Current entity type | `task` |
| `{{old.property}}` | Previous value (on update) | `backlog` |
| `{{new.property}}` | New value (on update) | `in_progress` |

### Trigger Types

| Trigger | When it fires |
|---------|---------------|
| `created: true` | When a new entity is created |
| `property: X` + `becomes: Y` | When property X changes to value Y |
| `property: X` + `from: Y` | When property X changes from value Y |
| `relation_created: X` | When relation of type X is added |
| `relation_removed: X` | When relation of type X is removed |

### Action Types

| Action | Effect |
|--------|--------|
| `set: property` + `value: X` | Sets property to value X |
| `create_relation` | Creates a new relation |

### Validation Severity Levels

| Severity | Behavior |
|----------|----------|
| `error` | Blocks the save (CLI returns error, UI shows error) |
| `warning` | Allows save but shows warning message |

### Example: Enforcing Sprint Discipline

Add validations that enforce your team's workflow:

```yaml
automations:
  # Features need acceptance criteria before development
  feature-ready-check:
    ...
    validate:
      - check: acceptance_criteria != ""
        severity: error
        message: "Features must have acceptance criteria before marking ready"
      - check: estimate != ""
        severity: warning
        message: "Consider adding an estimate"
  ...
  task-needs-feature:
    ...
    on:
      ...
      becomes: in_progress
    validate:
      - check: has_incoming_relation("implementedBy")
        severity: warning
        message: "Task should be linked to a feature for traceability"
```

### Example: Retrospective Workflow

Automate the retrospective → improvement flow:

```yaml
automations:
  # Improvements need ownership
  improvement-ownership:
    ...
    validate:
      - check: owner != ""
        severity: warning
        message: "Assign an owner to track this improvement"

  # Track improvement completion
  improvement-done:
    name: Improvement Completion
    on:
      entity: improvement
      property: status
      becomes: done
...
After adding automations, test them:

```bash
# Create a task - should auto-set created_at and created_by
rela create task -t "Test automation"

# Check the task
rela show TASK-010

# Start the task - should auto-set started_at
rela update TASK-010 -P "status=in_progress"

# Try to mark a decision as decided without rationale (should fail)
rela create decision -t "Test decision"
rela update DEC-003 -P "status=decided"
# Error: Decided decisions must document rationale
```

### Automations in the Data Entry UI

When using the data entry web interface, automations run automatically:

- **Property changes**: Trigger when you save a form
- **Validations**: Show as error/warning banners before save
- **Auto-set values**: Populate fields automatically

This ensures consistency whether you use the CLI or web UI.
