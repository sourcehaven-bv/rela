...
- **Symlink-refused:** if `.rela/audit/` is a symlink, the backend
  disables itself and logs `audit.write_failed`. Entity writes still
  succeed; audit just goes quiet.
...
- **Rename:** `subject` is empty; `before` and `after` carry the
  entity identity diff.
...
- Audit failures (disk full, permission errors, symlinked dir) never
  ...
  `audit.write_failed` via slog and continues.
