...
and never logged. Obtaining the token (IdP service-account / device flow) is
out-of-band. On loopback/dev with no proxy, sync works without a token. See
[Sync § Deployment](sync.md#deployment-behind-an-oauth-proxy) for the proxy
configuration (`--skip-jwt-bearer-tokens`, audience/issuer alignment).
...
> Note: sync requires the remote to run the PostgreSQL backend (the manifest /
> change-feed is a postgres feature). Against a non-postgres server the manifest
> endpoint returns 501 and `pull` reports the server does not support sync.
